-- Schéma Final MySQL - Plateforme Réservation Vols
-- DB Setup
CREATE DATABASE IF NOT EXISTS dbAvion2 CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE dbAvion2;
...
-- Table reservations
-- Modèle hybride: réservations directes CLIENT ou via AGENCE
CREATE TABLE reservations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    numero_reservation VARCHAR(50) UNIQUE NOT NULL,

    -- Soit agence_id (réservation par agence) SOIT client_id (réservation directe)
    ...
    -- Paiement simulé pour projet école
    ...
    -- Contrainte: soit agence soit client (pas les deux, pas aucun)
    CONSTRAINT chk_reservation_type CHECK (
        (type_reservation = 'PAR_AGENCE' AND agence_id IS NOT NULL AND client_id IS NULL)
        OR
...
-- Table historique_statuts_reservations
...
-- Table historique_statuts_admin
...
-- Table demandes_vols
-- Demandes complexes où le client sollicite une agence (OPTIONNEL dans modèle hybride)
...
-- Table demandes_agences (demandes de partenariat agences)
...
-- Table demandes_compagnies (demandes de partenariat compagnies)
...
-- Table messages_contact (messages de contact depuis la landing)
